...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--radar-csv", type=Path, required=True)
...
def main() -> None:
    ...
    for idx, img_path in enumerate(images, start=1):
        ...
        for i_box, box in enumerate(boxes):
            ...
            det = {
                "status": "ok",
                "label_source": "image_dominant_pseudo_gt",
        ...
        out_frames.append(
            {
                "frame_index": idx,
                "image": str(img_path),
                "image_timestamp": float(ts),
                "radar_timestamp": float(frame.timestamp_sec),
                "time_delta_sec": float(ts_radar - frame.timestamp_sec),
                "status": "ok",
                "selected_label_source": "image_dominant_pseudo_gt",
    ...
    payload = {
        "meta": {
            ...
            "label_definition": "Vehicle 3D center candidate is estimated from image geometry inside the vehicle 2D box; radar is validation only.",
